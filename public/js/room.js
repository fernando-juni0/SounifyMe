...
document
  .getElementById("click-button-back")
...
socket.on("reqVote", (data) => {
  ...
  if (data.uid != uid) {
    ...
    document.getElementById("main-chat-row").innerHTML += ` 
            ...
            `;
    document
      .getElementById("vote-acept")
      .addEventListener("click", async () => {
        if (
          document.getElementById("vote-acept").getAttribute("disabled") ==
          "disabled"
        ) {
          return;
        }
        document.getElementById("main-chat-vote-votes-number").innerText =
          parseInt(
            document.getElementById("main-chat-vote-votes-number").textContent
          ) + 1;
        document
          .getElementById("vote-acept")
          .setAttribute("disabled", "disabled");
        document
          .getElementById("vote-recuse")
          .setAttribute("disabled", "disabled");
    ...
    document
      ...
      .addEventListener("click", async () => {
        if (
          document.getElementById("vote-recuse").getAttribute("disabled") ==
          "disabled"
        ) {
          return;
        }
...
document
  .getElementById("options-button-pause-music")
  .addEventListener("click", async () => {
    if (audioTag.src) {
      audioTag.pause();
...
document
  .getElementById("options-button-play-music")
  .addEventListener("click", async () => {
...
document
  .getElementById("options-button-next-music")
  .addEventListener("click", () => {
    initVote("/indexChange", "pular a musica atual", indexMusic + 1);
  });
document
  .getElementById("options-button-back-music")
  .addEventListener("click", () => {
...
document.querySelectorAll(".queue-col").forEach((element, index) => {
  element.addEventListener("click", (res) => {
    ...
    initVote(
      "/indexChange",
      `Iniciar a Musica ${
        element.querySelector(".queue-col-music-texts-music-h1").textContent
...
let menu = document.getElementById('optinosUser-containner-op')
      ...
      document.querySelectorAll(".queue-col").forEach((element, index) => {
        element.addEventListener("click", (res) => {
          ...
          initVote(
            "/indexChange",
            `Iniciar a Musica ${
              element.querySelector(".queue-col-music-texts-music-h1")
                .textContent
            } da fila de Reprodução`,
            index
          );
